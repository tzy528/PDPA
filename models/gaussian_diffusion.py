...
class GaussianDiffusion(nn.Module):
    ...
    def getAttack(self, model, x_start, steps, sampling_noise=False):
        ...
        if steps == 0:
            ...
            if sampling_noise:
                ...
                x_start = x_start +  0.5  * noise
                # x_start = x_start + 5 * noise
    ...
    def training_losses(self, model, x_start, reweight=False):
        batch_size, device = x_start.size(0), x_start.device            # batch, device:[400,34395], cuda
        ...
        # update Lt_history & Lt_count  recording loss to a [100,10] array. At most,each step can recorded 10
    ...
    def p_mean_variance(self, model, x, t):          #p_mean_variance(model, x_t, t)
        """
